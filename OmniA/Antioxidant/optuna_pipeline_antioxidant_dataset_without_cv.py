...
# I already give the train , and test data

#df_antioxidant_case_study=pd.read_csv("df_antioxidant_case_study.csv") #put the correct path to the csv file
...
def run_optuna_pipeline_optimization(storage, study_name, direction, n_trials, x_train, y_train, x_val, y_val, metric, task_type, save_top_n, n_voting_pipelines, trial_timeout):
    hpo = PipelineOptimization(storage=storage, study_name=study_name, direction=direction)
    hpo.optimize('omnia.proteins.light', n_trials=n_trials, x=x_train, y=y_train, x_val=x_val, y_val=y_val, metric=metric, task_type=task_type, save_top_n=save_top_n, n_voting_pipelines=n_voting_pipelines, trial_timeout=trial_timeout, gc_after_trial=True)
    return hpo

# Parameters for the optimization
storage = 'sqlite:///antioxidant_without_CV_ssh.db'
study_name = 'antioxidant_withou_CV'
direction = 'maximize'
n_trials = 100
metric = 'f1'
task_type = 'binary'
...
hpo = run_optuna_pipeline_optimization(storage, study_name, direction, n_trials, x_train, y_train, x_val, y_val, metric, task_type, save_top_n, n_voting_pipelines, trial_timeout)
